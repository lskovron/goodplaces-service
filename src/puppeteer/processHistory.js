...
const processHistory = async (eventErrors, venueErrors, dateString) => {
  if (eventErrors.length > 0)
    console.error(`(${eventErrors}) errors processing events for ${dateString}`);
  if (venueErrors.length > 0)
    console.error(`(${venueErrors}) errors processing venues for ${dateString}`);
  ...
  await createOrUpdateHistory(historyEvent)
    .then(() => {
